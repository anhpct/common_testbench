...
my $tb_name    = "testbench.v";                                                        #testbench name                                               
my $top_module = "top_module_name";                                                  #top module in testbench
    ...
    $count = scalar (@files);                                                        #counts the number of elements in the @file
      if  ($count == 0){                                                             #
		...
		foreach my $getpath (@files){
          ...
          copy($tb_name,$name) or die "copy failed num1: $!";                        #copy origin file and run on temporary file	  
